...
def evaluate_model_with_thresholds_cnn(model, X_test, y_test, thresholds):
    X_test_reshaped = X_test.reshape(-1, 1, 1, 1)  # Reshape to (num_samples, 1, 1, 1)
    probabilities = model.predict(X_test_reshaped).flatten()  # Assuming the last layer uses a sigmoid activation
...
for run in range(n_runs):
    ...
    for model_name, model in models:
        ...
        print('\n--------------CNN--------------------------------')
        scaler = StandardScaler()
        X = scaler.fit_transform(X_balanced)  # Adjust if more features
...
csv_generate_path = '../fyp data/under_sampling.csv'
