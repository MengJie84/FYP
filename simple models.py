...
df_resampled = pd.DataFrame(X_train_resampled, columns=['Prediction'])
df_resampled['Actual'] = y_train_resampled
...
def evaluate_model(model_name, model, X, y, kf):
    ...
    results['Accuracy'].append(np.mean(cv_scores))
    results['Precision'].append(precision)
    results['Recall'].append(recall)
    results['F1 Score'].append(f1)
    print(f"{model_name} - Accuracy: {np.mean(cv_scores)}, Precision: {precision}, Recall: {recall}, F1 Score: {f1}")


# Evaluate XGBoost
print('--------------------XGBoost------------------------')
evaluate_model('XGBoost', xgb.XGBClassifier(use_label_encoder=False, eval_metric='logloss'), X_balanced, y_balanced, kf)


# Evaluate Random Forest
print('\n---------------Ensemble Random Forest----------------------------')
evaluate_model('Random Forest', RandomForestClassifier(random_state=24), X_balanced, y_balanced, kf)

print('\n---------------Logistic Regression----------------------------')
logistic_model = LogisticRegression(solver='liblinear', random_state=42)
evaluate_model('Logistic Regression', logistic_model, X_balanced, y_balanced, kf)

print('\n---------------Decision Tree----------------------------')
tree_model = DecisionTreeClassifier(random_state=42)
evaluate_model('Decision Tree', tree_model, X_balanced, y_balanced, kf)

print('\n---------------Naive Bayes----------------------------')
nb_model = GaussianNB()
evaluate_model('Naive Bayes', nb_model, X_balanced, y_balanced, kf)

print('\n---------------K-NN----------------------------')
knn_model = KNeighborsClassifier()
evaluate_model('k-NN', knn_model, X_balanced, y_balanced, kf)

print('\n--------------CNN--------------------------------')
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X_balanced)
X_reshaped = X_scaled.reshape((-1, 1, 1, 1))
y_reshaped = y_balanced
...
for fold, (train_index, test_index) in enumerate(kf.split(X_reshaped), start=1):
    ...
    y_train_fold, y_test_fold = y_reshaped[train_index], y_reshaped[test_index]
    ...
    cnn_precisions.append(precision)
    cnn_recalls.append(recall)
...
cnn_mean_precision = np.mean(cnn_precisions)
cnn_mean_recall = np.mean(cnn_recalls)
cnn_mean_f1 = np.mean(cnn_f1_scores)

# Add the CNN mean scores to the results dictionary
results['Model'].append('CNN')
results['Accuracy'].append(cnn_mean_accuracy)
results['Precision'].append(cnn_mean_precision)
results['Recall'].append(cnn_mean_recall)
results['F1 Score'].append(cnn_mean_f1)

df_results = pd.DataFrame(results)
print(df_results.to_string(index=False))

csv_generate_path = './fyp data/simple_result.csv'
