...
for train_index, test_index in kf.split(X, y):
    ...
    # Compute ROC-AUC
    y_prob = xgb_model.predict_proba(X_test)[:, 1]  # Get probability of the positive class
    if len(np.unique(y_test)) > 1:  # If both classes are present
        roc_aucs.append(roc_auc_score(y_test, y_prob))
    else:
        roc_aucs.append(np.nan)  # If only one class is present in the test fold

# Replace NaN values in ROC AUC scores with the mean of non-NaN values
roc_aucs = [score if not np.isnan(score) else np.nanmean(roc_aucs) for score in roc_aucs]

# Print the average metrics across all cross-validation folds
print(f"Average Accuracy: {np.mean(accuracies):.4f}")
print(f"Average Precision: {np.mean(precisions):.4f}")
print(f"Average Recall: {np.mean(recalls):.4f}")
print(f"Average F1 Score: {np.mean(f1_scores):.4f}")
print(f"Average ROC-AUC: {np.nanmean(roc_aucs):.4f}")