...
def plot_evaluation_metrics(model_name, y_test, y_probs):
    ...
    plt.title(f'Evaluation Metrics vs Probability Thresholds - {model_name}')
...
def evaluate_model(model_name, model, X_train, y_train, X_test, y_test, kf):
    ...
    else:  # If model is a Keras model or another model that does not have predict_proba
    ...
    print(f"{model_name} - Accuracy: {accuracy}, Precision: {precision}, Recall: {recall}, F1 Score: {f1},"
          f" \nError Rate: {error_rate}, Specificity: {specificity}, AUC-ROC: {auc_roc}, False Positive Rate: {fpr}")
    ...
    plot_evaluation_metrics(model_name, y_test, y_pred_proba)

# Evaluate XGBoost
print('--------------------XGBoost------------------------')
evaluate_model('XGBoost', xgb.XGBClassifier(use_label_encoder=False, eval_metric='logloss'), X_balanced, y_balanced, X_test, y_test, kf)


# Evaluate Random Forest
print('\n---------------Ensemble Random Forest----------------------------')
evaluate_model('Random Forest', RandomForestClassifier(random_state=24), X_balanced, y_balanced, X_test, y_test, kf)

print('\n---------------Logistic Regression----------------------------')
logistic_model = LogisticRegression(solver='liblinear', random_state=42)
evaluate_model('Logistic Regression', logistic_model, X_balanced, y_balanced, X_test, y_test, kf)
...
for fold, (train_index, test_index) in enumerate(kf.split(X_reshaped), start=1):
    ...
    probabilities = model.predict(X_test_fold).flatten()  # Get probabilities for AUC
    ...
    # Store fold results
    cnn_metrics['Accuracy'].append(accuracy)
    cnn_metrics['Precision'].append(precision)
    cnn_metrics['Recall'].append(recall)
    cnn_metrics['F1 Score'].append(f1)
    cnn_metrics['Error Rate'].append(error_rate)
    cnn_metrics['Specificity'].append(specificity)
    cnn_metrics['AUC-ROC'].append(auc_roc)
    cnn_metrics['False Positive Rate'].append(fpr)
...
for metric in cnn_metrics:
    cnn_metrics[metric] = np.mean(cnn_metrics[metric])

# Add the mean scores to the results dictionary
...
# Save to CSV
csv_generate_path = '../fyp data/evaluate_result.csv'
