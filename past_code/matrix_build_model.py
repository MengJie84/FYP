...
X_filtered = X[~is_outlier.any(axis=1)]
y_filtered = y[~is_outlier.any(axis=1)]
...
sns.boxplot(x=X['Prediction'])
plt.title('Boxplot of Prediction Feature Before Removing Outliers')
plt.show()

# after remove
plt.figure(figsize=(10, 6))
sns.boxplot(x=X_filtered['Prediction'])
plt.title('Boxplot of Prediction Feature After Removing Outliers')
plt.show()
...
print(class_counts)
def evaluate_model(model, X_resampled, y_resampled, kf):
...
evaluate_model(xgb_model, X_resampled, y_resampled, kf)

print('\n---------------Ensemble Random Forest----------------------------')
rf_model = RandomForestClassifier(random_state=42)
evaluate_model(rf_model,X_resampled, y_resampled, kf)
