...
for fold, (train_index, test_index) in enumerate(kf.split(X_reshaped), start=1):
    ...
    cnn_error_rate.append(error_rate)
    cnn_spec.append(specificity)
    cnn_auc.append(auc_roc)
    cnn_fpr.append(fpr)
...
cnn_mean_error = np.mean(cnn_error_rate)
cnn_mean_spec = np.mean(cnn_spec)
cnn_mean_auc = np.mean(cnn_auc)
cnn_mean_fpr = np.mean(cnn_fpr)
...
results['Error Rate'].append(cnn_mean_error)
results['Specificity'].append(cnn_mean_spec)
results['AUC-ROC'].append(cnn_mean_auc)
results['False Positive Rate'].append(cnn_mean_fpr)
