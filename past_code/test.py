...
def evaluate_model(model_name, model, X_train, y_train, X_test, y_test, kf):
    ...
    print(f"{model_name} - Test Accuracy: {accuracy}, Precision: {precision}, Recall: {recall}, F1 Score: {f1}")


# Evaluate XGBoost
print('--------------------XGBoost------------------------')
evaluate_model('XGBoost', xgb.XGBClassifier(use_label_encoder=False, eval_metric='logloss'), X_train_resampled, y_train_resampled, X_test, y_test, kf)


# Evaluate Random Forest
print('\n---------------Ensemble Random Forest----------------------------')
evaluate_model('Random Forest', RandomForestClassifier(random_state=24), X_train_resampled, y_train_resampled, X_test, y_test, kf)

print('\n---------------Logistic Regression----------------------------')
logistic_model = LogisticRegression(solver='liblinear', random_state=42)
evaluate_model('Logistic Regression', logistic_model, X_train_resampled, y_train_resampled, X_test, y_test, kf)

print('\n---------------Decision Tree----------------------------')
tree_model = DecisionTreeClassifier(random_state=42)
evaluate_model('Decision Tree', tree_model, X_train_resampled, y_train_resampled, X_test, y_test, kf)
...
for fold, (train_index, test_index) in enumerate(kf.split(X_reshaped), start=1):
    ...
    model.fit(X_train_fold, y_train_fold, epochs=10, batch_size=32, verbose=1)  # Consider showing progress
    ...
    f1 = f1_score(y_test_fold, predictions, zero_division=0)
